    ...
    try {
      ...
      let query = sql`
        SELECT 
          ...
          house_symbol_no,
          date_of_interview
        FROM acme_pariwartan_households
        WHERE profile_id = 'pariwartan'
      `;
...
export const getHouseholdByIdProcedure = protectedProcedure
  ...
  .query(async ({ ctx, input }) => {
    try {
      ...
      let query = sql`
        SELECT * FROM acme_pariwartan_households
    ...
    try {
      ...
      const query = sql`
        SELECT 
          ...
          h.ward_no,
          h.locality
        FROM acme_pariwartan_household_edit_requests e
        JOIN acme_pariwartan_households h ON h.id = e.household_id
      ...
      const countQuery = sql`
        SELECT COUNT(*) as total 
        FROM acme_pariwartan_household_edit_requests
      `;
