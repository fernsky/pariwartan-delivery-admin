...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    Object.entries(fuelGroups).forEach(([fuel, items]: [string, any]) => {
      const fuelTotal = items.reduce((sum: number, item: any) => sum + item.households, 0);
      fuelTypeTotals[fuel] = fuelTotal;
      totalHouseholds += fuelTotal;
    });

    // Find most common and cleanest fuel types
    const mostCommonFuel = Object.entries(fuelTypeTotals)
    ...
    const cleanFuelPercentage = ((cleanFuelTotal / totalHouseholds) * 100).toFixed(2);

    // Create rich keywords
    const keywordsNP = [
      "खजुरा गाउँपालिका खाना पकाउने इन्धन",
    ...
    // Create description
    const descriptionNP = `खजुरा गाउँपालिकामा खाना पकाउने इन्धन प्रयोगको विश्लेषण। कुल ${localizeNumber(totalHouseholds.toLocaleString(), "ne")} घरधुरी मध्ये ${localizeNumber(cleanFuelPercentage, "ne")}% घरधुरीले स्वच्छ इन्धन (एल.पी. ग्याँस, विद्युत, गोबर ग्याँस) प्रयोग गर्दछन्।`;
    ...
    return {
      title: "खाना पकाउने इन्धनको प्रयोगको अवस्था | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
...
const toc = [
  ...
  { level: 2, text: "खाना पकाउने इन्धनको वितरण", slug: "distribution-of-cooking-fuel" },
  { level: 2, text: "वडा अनुसार खाना पकाउने इन्धनको प्रयोग", slug: "ward-wise-cooking-fuel-usage" },
  { level: 2, text: "इन्धन प्रयोगको विश्लेषण", slug: "cooking-fuel-usage-analysis" },
  { level: 2, text: "स्वच्छ इन्धन प्रवर्द्धन रणनीति", slug: "clean-fuel-promotion-strategy" },
];
...
export default async function WardWiseCookingFuelPage() {
  ...
  Object.keys(fuelTypeTotals).forEach(fuel => {
    fuelTypePercentages[fuel] = parseFloat(((fuelTypeTotals[fuel] / totalHouseholds) * 100).toFixed(2));
  });
  ...
  const pieChartData = Object.entries(COOKING_FUEL_CATEGORIES).map(([categoryKey, category]) => {
    return {
      name: category.name,
  ...
  const wardWiseData = wardNumbers.map((wardNumber) => {
    ...
    Object.keys(COOKING_FUEL_CATEGORIES).forEach(categoryKey => {
      const category = COOKING_FUEL_CATEGORIES[categoryKey as keyof typeof COOKING_FUEL_CATEGORIES];
  ...
  const wardCleanFuelPercentages = wardWiseData.map((ward: any) => {
    const cleanFuelHouseholds = cleanFuels.reduce((sum, fuel) => {
      const fuelName = COOKING_FUEL_CATEGORIES[fuel as keyof typeof COOKING_FUEL_CATEGORIES].name;
  ...
  const bestWard = [...wardCleanFuelPercentages].sort((a, b) => b.percentage - a.percentage)[0];
  const worstWard = [...wardCleanFuelPercentages].sort((a, b) => a.percentage - b.percentage)[0];

  // Calculate clean fuel usage index (0-100, higher is better)
  const cleanFuelTotal = cleanFuels.reduce((sum, fuel) => sum + (fuelTypeTotals[fuel] || 0), 0);
  const traditionalFuelTotal = traditionalFuels.reduce((sum, fuel) => sum + (fuelTypeTotals[fuel] || 0), 0);
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="खाना पकाउने इन्धनको प्रयोगको अवस्था - खजुरा गाउँपालिका (Cooking Fuel Usage - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h1 className="scroll-m-20 tracking-tight mb-6">
              खजुरा गाउँपालिकामा खाना पकाउने इन्धनको प्रयोगको अवस्था
            </h1>
            ...
            <p>
              खाना पकाउने इन्धनको प्रयोग एक महत्वपूर्ण भौतिक सूचक हो जसले समुदायको जीवनस्तर, वातावरणीय स्वास्थ्य र 
              स्वच्छ ऊर्जामा पहुँचको अवस्थालाई दर्शाउँछ। खाना पकाउनको लागि प्रयोग गरिने इन्धनको प्रकारले स्वास्थ्यमा प्रत्यक्ष प्रभाव पार्नुका साथै 
              वातावरणीय दिगोपना र आर्थिक-सामाजिक अवस्थाको पनि संकेत गर्दछ। यस खण्डमा खजुरा गाउँपालिकाको विभिन्न वडाहरूमा 
              प्रयोग हुने खाना पकाउने इन्धनको विस्तृत विश्लेषण प्रस्तुत गरिएको छ।
            </p>
            <p>
              खजुरा गाउँपालिकामा कुल {localizeNumber(totalHouseholds.toLocaleString(), "ne")} घरधुरी मध्ये 
              {localizeNumber(fuelTypePercentages.LP_GAS?.toFixed(2) || "0.00", "ne")}% घरधुरीले एल.पी. ग्याँस, 
              {localizeNumber(fuelTypePercentages.WOOD?.toFixed(2) || "0.00", "ne")}% घरधुरीले दाउरा/काठ/कोइला, र 
              {localizeNumber(fuelTypePercentages.BIOGAS?.toFixed(2) || "0.00", "ne")}% घरधुरीले गोबर ग्याँसको प्रयोग गर्दछन्।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकामा खाना पकाउने इन्धनको प्रयोगको वितरण निम्नानुसार रहेको छ:
            </p>
          </div>
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            <h2 id="cooking-fuel-usage-analysis" className="scroll-m-20 border-b pb-2">
              खाना पकाउने इन्धनको प्रयोगको विश्लेषण
            </h2>
            <p>
              खजुरा गाउँपालिकामा खाना पकाउने इन्धनको प्रयोगको विश्लेषण गर्दा, समग्रमा 
              {localizeNumber(cleanFuelPercentage.toFixed(2), "ne")}% घरधुरीले स्वच्छ इन्धन (एल.पी. ग्याँस, विद्युत, गोबर ग्याँस) प्रयोग गर्दछन्।
              वडागत रूपमा हेर्दा वडा नं. {localizeNumber(bestWard.wardNumber.toString(), "ne")} मा 
              सबैभन्दा बढी स्वच्छ इन्धन प्रयोग गरिएको छ, जहाँ {localizeNumber(bestWard.percentage.toFixed(2), "ne")}% 
            ...
            <p>
              खजुरा गाउँपालिकामा खाना पकाउने इन्धनको प्रयोगको तथ्याङ्क विश्लेषणबाट निम्न रणनीतिहरू 
              अवलम्बन गर्न सकिन्छ:
            </p>
            ...
            <div className="pl-6 space-y-4">
              <div className="flex">
                ...
                <div>
                  <strong>स्वच्छ इन्धन सब्सिडी कार्यक्रम:</strong> {localizeNumber((100 - cleanFuelPercentage).toFixed(2), "ne")}% 
                  घरधुरीहरूले अझै परम्परागत इन्धन प्रयोग गर्ने हुनाले त्यस्ता घरधुरीहरूलाई एल.पी. ग्याँस, विद्युतीय चुल्हो, वा बायोग्यास जडानमा अनुदान तथा सहुलियत उपलब्ध गराउने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">२.</span>
                <div>
                  <strong>सुधारिएको चुल्हो कार्यक्रम विस्तार:</strong> वडा नं. {localizeNumber(worstWard.wardNumber.toString(), "ne")} मा स्वच्छ इन्धनको प्रयोग अत्यन्त न्यून भएकोले
                  त्यस क्षेत्रमा सुधारिएको चुल्हो वितरण र प्रशिक्षण कार्यक्रम संचालन गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">३.</span>
                <div>
                  <strong>बायोग्यास प्रवर्द्धन:</strong> पशुपालन गर्ने घरधुरीहरूमा बायोग्यास प्लान्ट जडान गर्न प्राविधिक र आर्थिक सहयोग उपलब्ध गराई स्वच्छ इन्धनको प्रयोगलाई बढावा दिने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">४.</span>
                <div>
                  <strong>विद्युतीय चुल्हो प्रवर्द्धन:</strong> विद्युत पहुँच भएका तर अझै परम्परागत इन्धन प्रयोग गर्ने घरधुरीहरूमा विद्युतीय चुल्हो प्रयोगको लाभ, सुरक्षित प्रयोग विधि र महत्व बारे जनचेतना अभिवृद्धि गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">५.</span>
                <div>
                  <strong>नवीकरणीय ऊर्जा प्रवर्द्धन:</strong> सौर्य ऊर्जा, वायु ऊर्जा आदि नवीकरणीय स्रोतहरूको विकास र विस्तारमा लगानी बढाई स्थानीय स्तरमा विद्युत उत्पादन र वितरण प्रणाली सुदृढ गर्न सहयोग गर्ने।
                </div>
              </div>
            </div>

            <p className="mt-6">
              यसरी खजुरा गाउँपालिकामा खाना पकाउने इन्धनको प्रयोगको विश्लेषणले पालिकामा स्वच्छ र वातावरणमैत्री इन्धनको पहुँच र प्रयोग बढाउनका लागि 
              लक्षित कार्यक्रम संचालन गर्न आवश्यक सूचना उपलब्ध गराउँछ। यसले घरभित्रको वायु प्रदूषण कम गरी महिला तथा बालबालिकाको स्वास्थ्य सुधार, वन विनाश कम गर्ने र समग्र पर्यावरणीय सन्तुलन कायम गर्न सहयोग पुर्‍याउँछ।
            </p>
          </div>
        </section>
      </div>
