...
export default function WardWiseTimeToPublicTransportSEO({
  ...
  const generateStructuredData = () => {
    ...
    const accessStats = wardNumbers.map((wardNumber) => {
      const wardData = timeToPublicTransportData.filter((item) => item.wardNumber === wardNumber);
      ...
      const quickAccessHouseholds = wardData
        .filter((item) => quickAccessCategories.includes(item.timeToPublicTransport))
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Time to Public Transport in Khajura Rural Municipality (खजुरा गाउँपालिका)",
      ...
      about: [
        ...
        },
        {
          ...
          description: "Time it takes for households to reach public transportation"
        }
      ],
      isBasedOn: {
