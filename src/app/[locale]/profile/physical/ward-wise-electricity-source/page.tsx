...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    Object.entries(sourceGroups).forEach(([source, items]: [string, any]) => {
      const sourceTotal = items.reduce((sum: number, item: any) => sum + item.households, 0);
      sourceTypeTotals[source] = sourceTotal;
      totalHouseholds += sourceTotal;
    });

    // Find most common electricity source
    const mostCommonSource = Object.entries(sourceTypeTotals)
      .sort(([, a]: [string, number], [, b]: [string, number]) => b - a)[0][0];
    
    const mainGridPercentage = ((sourceTypeTotals.ELECTRICITY || 0) / totalHouseholds * 100).toFixed(2);
    const solarPercentage = ((sourceTypeTotals.SOLAR || 0) / totalHouseholds * 100).toFixed(2);

    // Create rich keywords
    const keywordsNP = [
      "खजुरा गाउँपालिका विद्युत स्रोत",
    ...
    // Create description
    const descriptionNP = `खजुरा गाउँपालिकामा विद्युतको स्रोत प्रयोगको विश्लेषण। कुल ${localizeNumber(totalHouseholds.toLocaleString(), "ne")} घरधुरी मध्ये ${localizeNumber(mainGridPercentage, "ne")}% घरधुरीले केन्द्रीय विद्युत प्रणाली र ${localizeNumber(solarPercentage, "ne")}% घरधुरीले सौर्य ऊर्जा प्रयोग गर्दछन्।`;
    ...
    return {
      title: "विद्युतको स्रोतको प्रयोगको अवस्था | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
...
const toc = [
  ...
  { level: 2, text: "विद्युतको स्रोतको वितरण", slug: "distribution-of-electricity-source" },
  { level: 2, text: "वडा अनुसार विद्युतको स्रोतको प्रयोग", slug: "ward-wise-electricity-source-usage" },
  { level: 2, text: "विद्युत प्रयोगको विश्लेषण", slug: "electricity-usage-analysis" },
  { level: 2, text: "विद्युत पहुँच विस्तार रणनीति", slug: "electricity-expansion-strategy" },
];
...
export default async function WardWiseElectricitySourcePage() {
  ...
  Object.keys(sourceTypeTotals).forEach(source => {
    sourceTypePercentages[source] = parseFloat(((sourceTypeTotals[source] / totalHouseholds) * 100).toFixed(2));
  });
  ...
  const pieChartData = Object.entries(ELECTRICITY_SOURCE_CATEGORIES).map(([categoryKey, category]) => {
    return {
      name: category.name,
  ...
  const wardWiseData = wardNumbers.map((wardNumber) => {
    ...
    Object.keys(ELECTRICITY_SOURCE_CATEGORIES).forEach(categoryKey => {
      const category = ELECTRICITY_SOURCE_CATEGORIES[categoryKey as keyof typeof ELECTRICITY_SOURCE_CATEGORIES];
  ...
  const wardModernSourcePercentages = wardWiseData.map((ward: any) => {
    const modernSourceHouseholds = modernSources.reduce((sum, source) => {
      const sourceName = ELECTRICITY_SOURCE_CATEGORIES[source as keyof typeof ELECTRICITY_SOURCE_CATEGORIES].name;
  ...
  const bestWard = [...wardModernSourcePercentages].sort((a, b) => b.percentage - a.percentage)[0];
  const worstWard = [...wardModernSourcePercentages].sort((a, b) => a.percentage - b.percentage)[0];
  ...
  const traditionalSourceTotal = traditionalSources.reduce((sum, source) => sum + (sourceTypeTotals[source] || 0), 0);
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="विद्युतको स्रोतको प्रयोगको अवस्था - खजुरा गाउँपालिका (Electricity Source Usage - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h1 className="scroll-m-20 tracking-tight mb-6">
              खजुरा गाउँपालिकामा विद्युतको स्रोतको प्रयोगको अवस्था
            </h1>
            ...
            <p>
              विद्युतको स्रोत एक महत्वपूर्ण भौतिक सूचक हो जसले समुदायको जीवनस्तर, आधुनिकीकरण र 
              गुणस्तरीय ऊर्जामा पहुँचको अवस्थालाई दर्शाउँछ। विद्युतको उपलब्धता र त्यसको स्रोतको प्रकारले
              घरधुरीको जीवनयापन, स्वास्थ्य, शिक्षा, सूचना प्रविधिको पहुँच र व्यापारिक गतिविधिमा प्रत्यक्ष प्रभाव पार्दछ।
              यस खण्डमा खजुरा गाउँपालिकाको विभिन्न वडाहरूमा 
              प्रयोग हुने विद्युतको स्रोतको विस्तृत विश्लेषण प्रस्तुत गरिएको छ।
            </p>
            <p>
              खजुरा गाउँपालिकामा कुल {localizeNumber(totalHouseholds.toLocaleString(), "ne")} घरधुरी मध्ये 
              {localizeNumber(sourceTypePercentages.ELECTRICITY?.toFixed(2) || "0.00", "ne")}% घरधुरीले केन्द्रीय विद्युत प्रणाली, 
              {localizeNumber(sourceTypePercentages.SOLAR?.toFixed(2) || "0.00", "ne")}% घरधुरीले सौर्य ऊर्जा, र 
              {localizeNumber(sourceTypePercentages.KEROSENE?.toFixed(2) || "0.00", "ne")}% घरधुरीले मट्टितेलको प्रयोग गर्दछन्।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकामा विद्युतको स्रोतको प्रयोगको वितरण निम्नानुसार रहेको छ:
            </p>
          </div>
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            <h2 id="electricity-usage-analysis" className="scroll-m-20 border-b pb-2">
              विद्युतको स्रोतको प्रयोगको विश्लेषण
            </h2>
            <p>
              खजुरा गाउँपालिकामा विद्युतको स्रोतको प्रयोगको विश्लेषण गर्दा, समग्रमा 
              {localizeNumber(modernSourcePercentage.toFixed(2), "ne")}% घरधुरीले आधुनिक विद्युतीय स्रोत (केन्द्रीय विद्युत प्रणाली, सौर्य ऊर्जा) प्रयोग गर्दछन्।
              वडागत रूपमा हेर्दा वडा नं. {localizeNumber(bestWard.wardNumber.toString(), "ne")} मा 
              सबैभन्दा बढी आधुनिक विद्युतीय स्रोत प्रयोग गरिएको छ, जहाँ {localizeNumber(bestWard.percentage.toFixed(2), "ne")}% 
            ...
            <p>
              खजुरा गाउँपालिकामा विद्युतको स्रोतको प्रयोगको तथ्याङ्क विश्लेषणबाट निम्न रणनीतिहरू 
              अवलम्बन गर्न सकिन्छ:
            </p>
            ...
            <div className="pl-6 space-y-4">
              <div className="flex">
                ...
                <div>
                  <strong>ग्रिड विद्युत विस्तार कार्यक्रम:</strong> {localizeNumber((100 - sourceTypePercentages.ELECTRICITY || 0).toFixed(2), "ne")}% 
                  घरधुरीहरूले अझै केन्द्रीय विद्युत प्रणाली प्रयोग नगर्ने भएकाले त्यस्ता क्षेत्रहरूमा 
                  विद्युत विस्तारका लागि स्थानीय वितरण प्रणालीलाई सुदृढ गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">२.</span>
                <div>
                  <strong>वैकल्पिक ऊर्जा प्रवर्द्धन:</strong> वडा नं. {localizeNumber(worstWard.wardNumber.toString(), "ne")} मा आधुनिक विद्युतीय स्रोतको पहुँच न्यून भएकोले
                  त्यस क्षेत्रमा सौर्य ऊर्जा प्रणाली जडान र प्रशिक्षण कार्यक्रम संचालन गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">३.</span>
                <div>
                  <strong>लघु तथा साना जलविद्युत विकास:</strong> स्थानीय स्तरमा लघु र साना जलविद्युत आयोजना निर्माण गरी विद्युत आपूर्ति बढाउने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">४.</span>
                <div>
                  <strong>सामुदायिक सौर्य प्रणाली:</strong> विद्युत पहुँच नभएका बस्तीमा सामुदायिक सौर्य प्रणाली स्थापना गरी बत्ति, मोबाइल चार्ज, टेलिभिजन जस्ता आधारभूत आवश्यकता पूरा गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">५.</span>
                <div>
                  <strong>विद्युत वितरण प्रणाली सुधार:</strong> विद्यमान विद्युत वितरण प्रणालीमा हुने चुहावट कम गर्ने र प्राविधिक सुधारद्वारा
            ...
            <p className="mt-6">
              यसरी खजुरा गाउँपालिकामा विद्युतको स्रोतको प्रयोगको विश्लेषणले पालिकामा विद्युत पहुँच र विद्युतीय सेवाको गुणस्तर बढाउनका लागि 
              लक्षित कार्यक्रम संचालन गर्न आवश्यक सूचना उपलब्ध गराउँछ। यसले मट्टितेल जस्ता परम्परागत स्रोतमा निर्भरता कम गरी स्वच्छ र भरपर्दो विद्युत स्रोतमा जनताको पहुँच बढाउन मद्दत गर्नेछ, जसबाट जनताको जीवनस्तर, स्वास्थ्य, शिक्षा र आर्थिक विकासमा सकारात्मक योगदान पुग्नेछ।
            </p>
          </div>
        </section>
      </div>
