...
export default function DeceasedPopulationSEO({
  ...
  const generateStructuredData = () => {
    ...
    const ageGroupStats = Object.entries(ageGroupData).map(
      ([ageGroup, data]) => ({
        "@type": "Observation",
        name: `${AGE_GROUP_NAMES_EN[ageGroup] || ageGroup} deceased population in pariwartan Rural Municipality`,
        ...
        description: `${data.total.toLocaleString()} deceased people in pariwartan Rural Municipality in the age group ${AGE_GROUP_NAMES_EN[ageGroup] || ageGroup} (${((data.total / totalDeceasedPopulation) * 100).toFixed(2)}% of total deceased population)`,
      }),
    );
    ...
    const genderStats = [
      {
        "@type": "Observation",
        name: `${GENDER_NAMES_EN.MALE} deceased population in pariwartan Rural Municipality`,
        observationDate: new Date().toISOString().split("T")[0],
        measuredProperty: {
          "@type": "PropertyValue",
        ...
        description: `${genderTotals.male.toLocaleString()} deceased ${GENDER_NAMES_EN.MALE} in pariwartan Rural Municipality (${((genderTotals.male / totalDeceasedPopulation) * 100).toFixed(2)}% of total deceased population)`,
      },
      {
        "@type": "Observation",
        name: `${GENDER_NAMES_EN.FEMALE} deceased population in pariwartan Rural Municipality`,
        observationDate: new Date().toISOString().split("T")[0],
        measuredProperty: {
          "@type": "PropertyValue",
        ...
        description: `${genderTotals.female.toLocaleString()} deceased ${GENDER_NAMES_EN.FEMALE} in pariwartan Rural Municipality (${((genderTotals.female / totalDeceasedPopulation) * 100).toFixed(2)}% of total deceased population)`,
      },
      {
        "@type": "Observation",
        name: `${GENDER_NAMES_EN.OTHER} deceased population in pariwartan Rural Municipality`,
        observationDate: new Date().toISOString().split("T")[0],
        measuredProperty: {
          "@type": "PropertyValue",
        ...
        description: `${genderTotals.other.toLocaleString()} deceased ${GENDER_NAMES_EN.OTHER} in pariwartan Rural Municipality (${((genderTotals.other / totalDeceasedPopulation) * 100).toFixed(2)}% of total deceased population)`,
      },
    ...
    const wardStats = Object.entries(wardData).map(([ward, data]) => ({
      "@type": "Observation",
      name: `Ward ${ward} deceased population in pariwartan Rural Municipality`,
      observationDate: new Date().toISOString().split("T")[0],
      measuredProperty: {
        "@type": "PropertyValue",
      ...
      measuredValue: data.total,
      description: `${data.total.toLocaleString()} deceased people in Ward ${ward} of pariwartan Rural Municipality (${((data.total / totalDeceasedPopulation) * 100).toFixed(2)}% of total deceased population)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Age and Gender Wise Deceased Population in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Mortality data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total deceased population of ${totalDeceasedPopulation.toLocaleString()}. The most affected age group is ${AGE_GROUP_NAMES_EN[mostAffectedAgeGroupKey] || mostAffectedAgeGroupKey} with ${mostAffectedAgeGroupTotal.toLocaleString()} deceased (${mostAffectedAgeGroupPercentage}%), and the most affected ward is Ward ${mostAffectedWardNumber} with ${mostAffectedWardTotal.toLocaleString()} deceased (${mostAffectedWardPercentage}%).`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/demographics/ward-age-gender-wise-deceased-population",
      creator: {
        "@type": "Organization",
