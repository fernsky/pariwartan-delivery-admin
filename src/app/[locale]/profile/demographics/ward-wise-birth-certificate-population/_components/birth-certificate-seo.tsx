...
export default function BirthCertificateSEO({
  ...
  const generateStructuredData = () => {
    ...
    const wardObservations = birthCertificateData.map((item) => {
      const total =
        (item.withBirthCertificate || 0) + (item.withoutBirthCertificate || 0);
      const coverageRate =
        total > 0
      ...
      return {
        "@type": "Observation",
        name: `Birth Certificate Status for Children Under 5 Years in Ward ${item.wardNumber} of pariwartan Rural Municipality`,
        ...
        description: `Ward ${item.wardNumber} of pariwartan Rural Municipality has ${item.withBirthCertificate} children with birth certificates and ${item.withoutBirthCertificate} without birth certificates, representing a ${coverageRate}% coverage rate.`,
      };
    });
    ...
    const highestWard = [...birthCertificateData].sort(
      ...
      wardNumber: 0,
      withBirthCertificate: 0,
      withoutBirthCertificate: 0,
    };
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Birth Certificate Status for Children Under 5 Years in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Birth certificate data for children under 5 years across ${wardNumbers.length} wards of pariwartan Rural Municipality. Total population: ${totalPopulation.toLocaleString()} children, with ${totalWithCertificate.toLocaleString()} having birth certificates (${overallCoverageRate}% coverage) and ${totalWithoutCertificate.toLocaleString()} without birth certificates.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/demographics/ward-wise-birth-certificate-population",
      creator: {
        "@type": "Organization",
