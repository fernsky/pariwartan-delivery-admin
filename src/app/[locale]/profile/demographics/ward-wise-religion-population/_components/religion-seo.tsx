...
export default function ReligionSEO({
  ...
  const generateStructuredData = () => {
    ...
    const religionStats = religionData.map((item) => ({
      "@type": "Observation",
      name: `${RELIGION_NAMES_EN[item.religionType] || item.religionType} population in pariwartan Rural Municipality`,
      ...
      description: `${item.totalPopulation.toLocaleString()} people in pariwartan Rural Municipality follow ${RELIGION_NAMES_EN[item.religionType] || item.religionType} religion (${(item.percentage / 100).toFixed(2)}% of total population)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Religious Demographics of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Religious population distribution data for pariwartan Rural Municipality with a total population of ${totalPopulation.toLocaleString()} people.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/demographics/ward-wise-religion-population",
      creator: {
        "@type": "Organization",
