...
export default function LanguageSEO({
  ...
  const generateStructuredData = () => {
    ...
    const languageStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${LANGUAGE_NAMES_EN[item.language] || item.language} speakers in pariwartan Rural Municipality`,
      ...
      description: `${item.population.toLocaleString()} people in pariwartan Rural Municipality speak ${LANGUAGE_NAMES_EN[item.language] || item.language} as their mother tongue (${((item.population / totalPopulation) * 100).toFixed(2)}% of total population)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Linguistic Demographics of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Mother tongue distribution data across ${wardIds.length} wards of pariwartan Rural Municipality with a total population of ${totalPopulation.toLocaleString()} people.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/demographics/ward-wise-mother-tongue-population",
      creator: {
        "@type": "Organization",
