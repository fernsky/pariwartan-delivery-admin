...
export default function CropDiseaseSEO({
  ...
  const generateStructuredData = () => {
    ...
    const cropDiseaseStats = cropSummary.map((item) => ({
      "@type": "Observation",
      name: `${CROP_TYPES_EN[item.crop] || item.crop} Diseases and Pests in pariwartan Rural Municipality`,
      ...
      description: `${item.totalIssues} agricultural issues affecting ${CROP_TYPES_EN[item.crop] || item.crop} in pariwartan Rural Municipality: ${item.diseasesCount} diseases (${item.majorDiseases.slice(0, 3).join(", ")}) and ${item.pestsCount} pests (${item.majorPests.slice(0, 3).join(", ")}). This represents ${((item.totalIssues / (totalPests + totalDiseases)) * 100).toFixed(2)}% of all crop issues in the municipality.`,
    }));
    ...
    return {
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/municipality-wide-crop-diseases",
      creator: {
        "@type": "Organization",
