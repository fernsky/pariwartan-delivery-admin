...
export default function RemittanceBarChart({
  ...
  const commonGroups = remittanceAmountGroupOptions.map(option => option.label);

  // Calculate total for each remittance group for sorting
  const groupTotals = commonGroups.reduce((acc, group) => {
  ...
  const sortedGroups = commonGroups.sort((a, b) => 
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        {sortedGroups.slice(0, 6).map((group, index) => {
          ...
          const amountGroupOption = remittanceAmountGroupOptions.find(option => option.label === group);
