...
export default function RemittanceSEO({
  ...
  const generateStructuredData = () => {
    ...
    const remittanceStats = overallSummary.slice(0, 10).map((item) => ({
      "@type": "Observation",
      name: `${item.amountGroupLabel} remittance from pariwartan Rural Municipality`,
      ...
      description: `${item.sendingPopulation.toLocaleString()} people from pariwartan Rural Municipality send remittance in the range of ${item.amountGroupLabel} (${((item.sendingPopulation / totalSendingPopulation) * 100).toFixed(2)}% of total remittance senders)`,
    }));
    ...
    return {
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/ward-wise-remittance",
      creator: {
        "@type": "Organization",
