...
export default function IncomeSourceSEO({
  ...
  const generateStructuredData = () => {
    ...
    const incomeSourceStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${INCOME_SOURCE_NAMES_EN[item.incomeSource] || item.incomeSource} households in pariwartan Rural Municipality`,
      ...
      description: `${item.households.toLocaleString()} households in pariwartan Rural Municipality depend on ${INCOME_SOURCE_NAMES_EN[item.incomeSource] || item.incomeSource} as their primary income source (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Household Income Sources of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Household income source distribution data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total of ${totalHouseholds.toLocaleString()} households.`,
      keywords: [
      ...
      url: "https://pariwartan-rm.gov.np/profile/economics/ward-wise-household-income-source",
      creator: {
        "@type": "Organization",
