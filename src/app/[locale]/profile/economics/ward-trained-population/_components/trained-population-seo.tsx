...
export default function TrainedPopulationSEO({
  ...
  const generateStructuredData = () => {
    ...
    const wardStats = trainedPopulationData.map((item) => ({
      "@type": "Observation",
      name: `Ward ${item.wardNumber} trained population in pariwartan Rural Municipality`,
      ...
      description: `${item.trainedPopulation.toLocaleString()} people in Ward ${item.wardNumber} of pariwartan Rural Municipality have received vocational or skills training (${((item.trainedPopulation / totalTrainedPopulation) * 100).toFixed(2)}% of total trained population)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Trained Population Distribution of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Trained population distribution data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total of ${totalTrainedPopulation.toLocaleString()} trained individuals.`,
      keywords: [
      ...
      url: "https://pariwartan-rm.gov.np/profile/economics/ward-trained-population",
      creator: {
        "@type": "Organization",
