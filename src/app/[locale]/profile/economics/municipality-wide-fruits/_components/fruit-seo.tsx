...
export default function FruitSEO({
  ...
  const generateStructuredData = () => {
    ...
    const fruitStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${FRUIT_TYPES_EN[item.type] || item.type} in Paribartan Rural Municipality`,
      ...
      description: `${item.production.toFixed(2)} tonnes of ${FRUIT_TYPES_EN[item.type] || item.type} produced in Paribartan Rural Municipality (${((item.production / totalProduction) * 100).toFixed(2)}% of total production). Sales volume: ${item.sales.toFixed(2)} tonnes. Revenue: NPR ${item.revenue.toLocaleString()}.`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Fruit Types in Paribartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Fruit production and sales statistics of Paribartan Rural Municipality with a total production of ${totalProduction.toFixed(
        2,
