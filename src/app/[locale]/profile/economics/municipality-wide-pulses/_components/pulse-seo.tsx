...
export default function PulseSEO({
  ...
  const generateStructuredData = () => {
    ...
    const pulseStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${PULSE_TYPES_EN[item.type] || item.type} in pariwartan Rural Municipality`,
      ...
      description: `${item.production.toFixed(2)} tonnes of ${PULSE_TYPES_EN[item.type] || item.type} produced in pariwartan Rural Municipality (${((item.production / totalProduction) * 100).toFixed(2)}% of total production). Sales volume: ${item.sales.toFixed(2)} tonnes. Revenue: NPR ${item.revenue.toLocaleString()}.`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Pulse Types in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Pulse production and sales statistics of pariwartan Rural Municipality with a total production of ${totalProduction.toFixed(2)} tonnes and sales of ${totalSales.toFixed(2)} tonnes (${((totalSales / totalProduction) * 100).toFixed(2)}% of production). The most common pulse crop is ${mostProducedPulseEN} with ${mostProducedPulse?.production.toFixed(2)} tonnes (${mostProducedPercentage}%). Self-consumption represents ${selfConsumptionPercentage}% of total production. Total revenue from pulse sales is NPR ${totalRevenue.toLocaleString()}.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/municipality-wide-pulses",
      creator: {
        "@type": "Organization",
