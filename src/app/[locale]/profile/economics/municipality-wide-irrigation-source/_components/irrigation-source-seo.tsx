...
export default function IrrigationSourceSEO({
  ...
  const generateStructuredData = () => {
    ...
    const irrigationSourceStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${IRRIGATION_SOURCE_TYPES_EN[item.type] || item.type} in pariwartan Rural Municipality`,
      ...
      description: `${item.coverage.toFixed(2)} hectares in pariwartan Rural Municipality are irrigated through ${IRRIGATION_SOURCE_TYPES_EN[item.type] || item.type} (${((item.coverage / totalCoverage) * 100).toFixed(2)}% of total irrigated area)`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Irrigation Source Types in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Irrigation source statistics of pariwartan Rural Municipality with a total irrigation coverage of ${totalCoverage.toFixed(2)} hectares. The most common irrigation source is ${mostCommonSourceEN} with ${mostCommonSource?.coverage.toFixed(2)} hectares (${mostCommonSourcePercentage}%). Traditional irrigation methods account for ${traditionalSourcePercentage}% of all irrigated area. Irrigation sustainability score is ${sustainabilityScore}%.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/municipality-wide-irrigation-source",
      creator: {
        "@type": "Organization",
