...
export default function VegetableSEO({
  ...
  const generateStructuredData = () => {
    ...
    const vegetableStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${VEGETABLE_TYPES_EN[item.type] || item.type} in pariwartan Rural Municipality`,
      ...
      description: `${item.production.toFixed(2)} tonnes of ${VEGETABLE_TYPES_EN[item.type] || item.type} produced in pariwartan Rural Municipality (${((item.production / totalProduction) * 100).toFixed(2)}% of total production). Sales volume: ${item.sales.toFixed(2)} tonnes. Revenue: NPR ${item.revenue.toLocaleString()}.`,
    }));
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Vegetable Types in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Vegetable production and sales statistics of pariwartan Rural Municipality with a total production of ${totalProduction.toFixed(
        2,
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/municipality-wide-vegetables",
      creator: {
        "@type": "Organization",
