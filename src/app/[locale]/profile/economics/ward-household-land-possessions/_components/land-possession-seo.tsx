...
export default function LandPossessionSEO({
  ...
  const generateStructuredData = () => {
    ...
    const landPossessionStats = landPossessionData.map((item) => ({
      "@type": "Observation",
      name: `Ward ${item.wardNumber} land-owning households in pariwartan Rural Municipality`,
      ...
      description: `${item.households.toLocaleString()} households in Ward ${item.wardNumber} of pariwartan Rural Municipality own land (${((item.households / totalHouseholdsWithLand) * 100).toFixed(2)}% of total land-owning households)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Household Land Ownership in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Land ownership distribution data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total of ${totalHouseholdsWithLand.toLocaleString()} land-owning households.`,
      keywords: [
      ...
      url: "https://pariwartan-rm.gov.np/profile/economics/ward-household-land-possessions",
      creator: {
        "@type": "Organization",
