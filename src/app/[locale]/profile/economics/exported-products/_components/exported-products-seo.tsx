...
export default function ExportedProductsSEO({
  ...
  const generateStructuredData = () => {
    ...
    const categoryStats = categoryDistribution.map((item) => ({
      "@type": "Observation",
      name: `${CATEGORY_NAMES_EN[item.name] || item.name} exports from pariwartan Rural Municipality`,
      ...
      } category are exported from pariwartan Rural Municipality (${item.percentage}% of total exports)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Exported Products Catalog of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Comprehensive catalog of ${totalProducts.toLocaleString()} products exported from pariwartan Rural Municipality, categorized by type and market potential.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/exported-products",
      creator: {
        "@type": "Organization",
