...
export default function IncomeSourcesSEO({
  ...
  const generateStructuredData = () => {
    ...
    const incomeSourceStats = overallSummary.map((item) => ({
      ...
      description: `${item.households.toLocaleString()} households in pariwartan Rural Municipality depend on ${INCOME_SOURCE_NAMES_EN[item.incomeSource] || item.incomeSource} (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households)`,
    }));
    ...
    return {
      ...
      description: `Household income sources distribution data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total of ${totalHouseholds.toLocaleString()} households.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/economics/income-sources",
      creator: {
        "@type": "Organization",
