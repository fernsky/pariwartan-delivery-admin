...
export default function SkillsSEO({
  ...
  const generateStructuredData = () => {
    ...
    const skillsStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${SKILL_NAMES_EN[item.skill] || item.skill} in pariwartan Rural Municipality`,
      ...
      description: `${item.population.toLocaleString()} people in pariwartan Rural Municipality have skills in ${SKILL_NAMES_EN[item.skill] || item.skill} (${((item.population / totalPopulation) * 100).toFixed(2)}% of total skilled population)`,
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Skills Distribution of pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Skills distribution data across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total skilled population of ${totalPopulation.toLocaleString()} people (${localizeNumber(totalPopulation.toString(), "ne")} व्यक्ति).`,
      keywords: [
      ...
      url: "https://pariwartan-rm.gov.np/profile/economics/ward-main-skills",
      creator: {
        "@type": "Organization",
