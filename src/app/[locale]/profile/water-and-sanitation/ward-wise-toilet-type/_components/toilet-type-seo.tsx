...
export default function ToiletTypeSEO({
  ...
  const generateStructuredData = () => {
    ...
    const toiletTypeStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${item.toiletTypeName} toilet facilities in pariwartan Rural Municipality`,
      ...
      description: `${item.households.toLocaleString()} households in pariwartan Rural Municipality use ${item.toiletTypeName} (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households)`,
    }));
    ...
    return {
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/water-and-sanitation/ward-wise-toilet-type",
      creator: {
        "@type": "Organization",
