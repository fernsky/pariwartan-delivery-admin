...
interface WardFormalEducationPieChartsProps {
...
export default function WardFormalEducationPieCharts({
  wardWiseData,
  FORMAL_EDUCATION_GROUPS,
}: WardFormalEducationPieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const formalEducationGroups = Object.values(FORMAL_EDUCATION_GROUPS)
          .map((group) => ({
            name: group.name,
        ...
        return (
          <div
            ...
            className="h-auto border rounded-md p-4"
          >
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {formalEducationGroups.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.color} />
                    ))}
                  </Pie>
                  <Tooltip
                    content={<CustomTooltip totalPopulation={total} />}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="mt-3">
              <div className="grid grid-cols-1 gap-2">
                {formalEducationGroups.map((item, i) => {
