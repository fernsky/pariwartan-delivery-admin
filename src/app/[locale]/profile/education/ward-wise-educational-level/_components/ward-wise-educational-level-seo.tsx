  ...
  const generateStructuredData = () => {
    ...
    const educationStats = wardNumbers
      .map((wardNumber) => {
        ...
        return {
          "@type": "Observation",
          name: `Educational Level Statistics in Ward ${wardNumber} of pariwartan Rural Municipality`,
          ...
          description: `In Ward ${wardNumber} of pariwartan Rural Municipality, ${higherEducation.toLocaleString()} people (${higherEducationPercent}%) have attained higher education out of a total of ${totalWardPopulation.toLocaleString()} people.`,
        };
      })
      .filter(Boolean);
    ...
    return {
      ...
      "@type": "Dataset",
      name: "Educational Level Distribution in pariwartan Rural Municipality (परिवर्तन गाउँपालिका)",
      description: `Analysis of educational levels across ${wardNumbers.length} wards of pariwartan Rural Municipality with a total population of ${totalPopulation.toLocaleString()}. ${educationGroupTotals.HIGHER_EDUCATION.toLocaleString()} people (${educationGroupPercentages.HIGHER_EDUCATION.toFixed(2)}%) have attained higher education. The highest educational level is in Ward ${bestEducatedWard?.wardNumber || ""} with ${bestEducatedWard?.percentage.toFixed(2) || ""}% of people having higher education.`,
      keywords: [
      ...
      url: "https://digital.pariwartanmun.gov.np/profile/education/ward-wise-educational-level",
      creator: {
        "@type": "Organization",
